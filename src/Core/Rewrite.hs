...
rewriteGo lv d book old neo val =
  case val of
    ...
    Let k t v f -> Let k (fmap (rewrite lv d book old neo) t) (rewrite lv d book old neo v) (\x -> rewrite lv d book old neo (f x))
    Set         -> Set
    ...
    Bt0         -> Bt0
    Bt1         -> Bt1
    BitM f t    -> BitM (rewrite lv d book old neo f) (rewrite lv d book old neo t)
    Nat         -> Nat
    Zer         -> Zer
    Suc n       -> Suc (rewrite lv d book old neo n)
    NatM z s    -> NatM (rewrite lv d book old neo z) (rewrite lv d book old neo s)
    Lst t       -> Lst (rewrite lv d book old neo t)
    Nil         -> Nil
    Con h t     -> Con (rewrite lv d book old neo h) (rewrite lv d book old neo t)
    LstM n c    -> LstM (rewrite lv d book old neo n) (rewrite lv d book old neo c)
    Enu s       -> Enu s
    Sym s       -> Sym s
    EnuM c e    -> EnuM (map (\(s,t) -> (s, rewrite lv d book old neo t)) c) (rewrite lv d book old neo e)
    Num t       -> Num t
    Val v       -> Val v
    ...
    SigM f      -> SigM (rewrite lv d book old neo f)
    ...
    Rfl         -> Rfl
    EqlM p f    -> EqlM (rewrite lv d book old neo p) (rewrite lv d book old neo f)
    ...
    SupM l f    -> SupM (rewrite lv d book old neo l) (rewrite lv d book old neo f)
