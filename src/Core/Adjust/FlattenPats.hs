...
peelCtrCol d span book (cut->k) ((((cut->p):ps),rhs):cs) = 
  ...
  case (k,p) of
    ...
    (Con _ _  , Var k _)   -> let headVar = var d; tailVar = var (d+1)
    ...
    (Loc sp (Tup _ _), Var k _) -> let xVar = var d; yVar = var (d+1)
                                   in (((xVar:yVar:ps), bindVarByName k (Tup xVar yVar) rhs) : picks , ((p:ps),rhs) : drops)
    (Tup _ _  , Var k _)   -> let xVar = var d; yVar = var (d+1)
                              in (((xVar:yVar:ps), bindVarByName k (Tup xVar yVar) rhs) : picks , ((p:ps),rhs) : drops)
    (Sym s    , Sym s' )
    ...
    (Sup l _ _, Var k _)   -> let aVar = var d; bVar = var (d+1)
