...
peelCtrCol d span book (cut->k) ((((cut->p):ps),rhs):cs) = 
  ...
  case (k,p) of
    ...
    (Con _ _  , Var k _)   -> let hVar = freshVarFromName k "h" 0; tVar = freshVarFromName k "t" 0
                              in (((hVar:tVar:ps), bindVarByName k (Con hVar tVar) rhs) : picks , ((p:ps),rhs) : drops)
    ...
    (Loc sp (Tup _ _), Var k _) -> let xVar = freshVarFromName k "x" 0; yVar = freshVarFromName k "y" 0
                                   in (((xVar:yVar:ps), bindVarByName k (Tup xVar yVar) rhs) : picks , ((p:ps),rhs) : drops)
    (Tup _ _  , Var k _)   -> let xVar = freshVarFromName k "x" 0; yVar = freshVarFromName k "y" 0
                              in (((xVar:yVar:ps), bindVarByName k (Tup xVar yVar) rhs) : picks , ((p:ps),rhs) : drops)
    (Sym s    , Sym s' )
    ...
    (Sup l _ _, Var k _)   -> let aVar = freshVarFromName k "a" 0; bVar = freshVarFromName k "b" 0
