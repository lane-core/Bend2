...
desugarPats :: Int -> Span -> Expr -> Expr
...
match :: Int -> Span -> Expr -> [Move] -> [Case] -> Expr
-- match x { 0n: z ; 1n+p: s }
-- ---------------------------
-- ~x { 0n: z ; 1n+: λp. s }
match d span x ms (([(cut -> Zer)], z) : ([(cut -> Suc p)], s) : _) =
 ...
 where
  if_zer = lam d (map fst ms) $ desugarPats d span z
...
match d span x ms (([(cut -> Suc p)], s) : ([(cut -> Var k i)], v) : _) =
  apps d (map snd ms) $ App (NatM if_zer if_suc) x
 where
  if_zer = lam d (map fst ms) $ desugarPats d span (bindVarByName k Zer v)
  if_suc = Lam (patOf d p) (Just Nat) $ \x -> lam d (map fst ms) $ desugarPats (d + 1) span s
...
match d span x ms (([(cut -> Nil)], n) : ([(cut -> Con h t)], c) : _) =
 ...
 where
  if_nil = lam d (map fst ms) $ desugarPats d span n
...
match d span x ms (([(cut -> Con h t)], c) : ([(cut -> Var k i)], v) : _) =
  apps d (map snd ms) $ App (LstM if_nil if_con) x
 where
  if_nil = lam d (map fst ms) $ desugarPats d span (bindVarByName k Nil v)
  if_con = Lam (patOf d h) Nothing $ \_ -> Lam (patOf (d + 1) t) Nothing $ \_ -> lam d (map fst ms) $ desugarPats (d + 2) span c
...
match d span x ms (([p], u) : _)
  | isUnitPat p =
      let mloc = unitPatLoc p
       in let body = lam d (map fst ms) $ desugarPats d span u
           in let uni = maybe (UniM body) (\sp -> Loc sp (UniM body)) mloc
               in apps d (map snd ms) $ App uni x
 where
  isUnitPat :: Expr -> Bool
  isUnitPat (cut -> One) = True
  isUnitPat _ = False
  unitPatLoc :: Expr -> Maybe Span
...
match d span x ms (([tup], p) : _)
  | isTupPat tup =
      let mloc = tupPatLoc tup
       in let (a, b) = tupPatFields tup
           in let if_tup = Lam (patOf d a) Nothing $ \_ -> Lam (patOf (d + 1) b) Nothing $ \_ -> lam d (map fst ms) $ desugarPats (d + 2) span p
               in let sigm = maybe (SigM if_tup) (\sp -> Loc sp (SigM if_tup)) mloc
                   in apps d (map snd ms) $ App sigm x
 where
  isTupPat :: Expr -> Bool
  isTupPat (cut -> Tup _ _) = True
  isTupPat _ = False
  tupPatLoc :: Expr -> Maybe Span
  tupPatLoc (Loc sp (cut -> Tup _ _)) = Just sp
  tupPatLoc _ = Nothing
  tupPatFields :: Expr -> (Expr, Expr)
...
match d span x ms (([(cut -> Sup l a b)], s) : _) =
 ...
 where
  if_sup = Lam (patOf d a) Nothing $ \_ -> Lam (patOf (d + 1) b) Nothing $ \_ -> lam d (map fst ms) $ desugarPats (d + 2) span s
