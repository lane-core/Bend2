...
-- PROBLEM: the file above is *not* handling Locations correctly. note that the
-- TypeMismatch constructors, and others, are being created without a Span. this is
-- wrong, and results in type errors. the file above has to be rewritten to use the
-- Span information. to do so, update infer, check, etc., to receive a 'span'
-- argument after the 'd' argument. then, update that argument whenever the term is
-- a Loc constructor. then, when creating TypeMismatch etc., use the current 'span'
-- rerwite the entire Check.hs file, from scratch, to include this change
