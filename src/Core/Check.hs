...
-- NOTE: we've added an extra field to the Cse constructor, representing a default case.

-- our goal is to update the type-checker above to handle it

-- here's how it should work:

-- - when we check a Cse, and all labels are present in the Enu type, then, the default case is checked as having type `ScrutineeType -> Unit` - i.e., the return type decays to Unit

-- - when we check a Cse, and some labels are absent in the Enu type, then, the default case is checked as having type `∀x:ScrutineeType. goal` - i.e., the return type stays the same

-- this allows the user to fill the default case with just '()' when all cases are covered

-- now, your goal is to rewrite the entire file above to update the Cse checker
