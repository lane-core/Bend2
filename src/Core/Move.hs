...
-- Auto-Move variables, linearizing/specializing them.
-- Currently, only enabled for the second field of pairs:
-- ~p{ (,): λx.λy.(~x { P0:B0 … Pn:Bn }) }
-- -----------------------------------------
-- ~p{ (,): λx.(~x { P0:λy.B0 … Pn:λy.Bn }) }

move :: Term -> Term
-- move t = t
move (Loc s t)    = Loc s (move t)
move (SigM x f)   = SigM (move x) (moveGet f)
move (Lam n b)    = Lam n (move . b)
move (App f x)    = App (move f) (move x)
move (Let v f)    = Let (move v) (move f)
move (Fix n b)    = Fix n (move . b)
move (Ann x t)    = Ann (move x) (move t)
move (Chk x t)    = Chk (move x) (move t)
move (BitM x f t) = BitM (move x) (move f) (move t)
move (NatM x z s) = NatM (move x) (move z) (move s)
move (LstM x n c) = LstM (move x) (move n) (move c)
move (EnuM x c d) = EnuM (move x) [ (tag, move br) | (tag,br) <- c ] (move d)
move (UniM x f)   = UniM (move x) (move f)
move (Sig a b)    = Sig (move a) (move b)
move (All a b)    = All (move a) (move b)
move (Tup a b)    = Tup (move a) (move b)
move (Con h t)    = Con (move h) (move t)
...
move (Lst t)      = Lst (move t)
move (Eql t a b)  = Eql (move t) (move a) (move b)
move (EqlM x f)   = EqlM (move x) (move f)
move (Ind t)      = Ind (move t)
move (Frz t)      = Frz (move t)
move (Sup l a b)  = Sup l (move a) (move b)
move (Met n t x)  = Met n (move t) (map move x)
...
putLam :: Name -> Term -> Term
putLam y term = case term of
  Loc s t    -> Loc s (putLam y t)
  UniM x f   -> UniM x (putLamAfter 0 y f)
  BitM x t f -> BitM x (putLamAfter 0 y t) (putLamAfter 0 y f)
  NatM x z s -> NatM x (putLamAfter 0 y z) (putLamAfter 1 y s)
  LstM x n c -> LstM x (putLamAfter 0 y n) (putLamAfter 2 y c)
  EnuM x c d -> EnuM x [ (tag, putLamAfter 0 y b) | (tag, b) <- c ] (putLamAfter 0 y d)
  EqlM x f   -> EqlM x (putLamAfter 0 y f)
  SigM x f   -> SigM x (putLamAfter 2 y f)
  t          -> t
  where putLamAfter :: Int -> Name -> Term -> Term
        putLamAfter 0 y t         = Lam y $ \x -> t
        putLamAfter n y (Lam k f) = Lam k $ \x -> putLamAfter (n-1) y (f x)
        putLamAfter n y t         = Lam k $ \x -> putLamAfter (n-1) y (App t (Var k 0))
                          where k = "_" ++ show n
