...
showPlain :: S.Set String -> Expr -> Int -> String
...
showFix shadowed k f depth = "μ" ++ kStr ++ ". " ++ showPlain shadowed (f (Var k depth)) (depth + 1)
 where
  kStr = varName shadowed k depth

-- | x : T = v body or x = v body
showLet :: S.Set String -> String -> Maybe Expr -> Expr -> Body -> Int -> String
...
-- | use x = v body
showUse :: S.Set String -> String -> Expr -> Body -> Int -> String
...
showSuc :: S.Set String -> Expr -> Int -> String
...
showCon :: S.Set String -> Expr -> Expr -> Int -> String
showCon shadowed h t depth = fromMaybe plain (showStr shadowed (Con h t) depth)
 where
  plain = showPlain shadowed h depth ++ "<>" ++ showPlain shadowed t depth

-- | Enum matcher: λ{&foo:x;&bar:y;default}
showEnuM :: S.Set String -> [(String, Expr)] -> Expr -> Int -> String
showEnuM shadowed cs d depth = "λ{" ++ intercalate ";" cases ++ ";" ++ showPlain shadowed d depth ++ "}"
 where
  cases = map (\(s, t) -> "&" ++ s ++ ":" ++ showPlain shadowed t depth) cs

-- | Dependent pair type: Σx:A. B or A & B
showSig :: S.Set String -> Expr -> Expr -> Int -> String
...
showAll :: S.Set String -> Expr -> Expr -> Int -> String
...
showLam :: S.Set String -> String -> Maybe Expr -> Body -> Int -> String
...
showApp :: S.Set String -> Expr -> Int -> String
...
showTup :: S.Set String -> Expr -> Int -> String
showTup shadowed term depth = fromMaybe plain (showCtr term)
 where
  plain = "(" ++ intercalate "," (map (\t -> showPlain shadowed t depth) (flattenTup term)) ++ ")"

-- | Equality type: T{a == b}
showEql :: S.Set String -> Expr -> Expr -> Expr -> Int -> String
...
-- | Binary operator: (a + b)
showOp2 :: S.Set String -> NOp2 -> Expr -> Expr -> Int -> String
...
showOp1 :: S.Set String -> NOp1 -> Expr -> Int -> String
...
showPat :: S.Set String -> [Expr] -> [Move] -> [Case] -> Int -> String
...
showVal v = case v of
  ...
  I64_V n -> case n >= 0 of
    ...
    False -> show n
  F64_V n -> show n
  CHR_V c -> "'" ++ Core.Legacy.Show.showChar c ++ "'"
...
showStr :: S.Set String -> Expr -> Int -> Maybe String
...
adjustDepths :: Expr -> Int -> Expr
adjustDepths term depth = case term of
  Var k i -> Var k i
  Ref k i -> Ref k i
  Sub t -> Sub (adjustDepths t depth)
  Fix k f -> Fix k (\x -> adjustDepths (f (Var k depth)) (depth + 1))
  Let k t v f -> Let k (fmap (\t' -> adjustDepths t' depth) t) (adjustDepths v depth) (const (adjustDepths (f (Var k depth)) (depth + 1)))
...
instance Show Book where
  show (Book defs names) = unlines [showDefn name (defs M.! name) | name <- names]
   where
    showDefn k (_, x, t) = k ++ " : " ++ show t ++ " = " ++ showExpr True x
...
instance Show Error where
  show err = case err of
    ...
    TypeMismatch span ctx goal typ hint -> "\x1b[1mMismatch:\x1b[0m\n- Goal: " ++ showExpr True goal ++ "\n- Type: " ++ showExpr True typ ++ "\n" ++ showHint hint ++ "\x1b[1mContext:\x1b[0m\n" ++ show ctx ++ show span
    ExprMismatch span ctx a b hint -> "\x1b[1mMismatch:\x1b[0m\n- " ++ showExpr True a ++ "\n- " ++ showExpr True b ++ "\n" ++ showHint hint ++ "\x1b[1mContext:\x1b[0m\n" ++ show ctx ++ show span
    IncompleteMatch span ctx hint -> "\x1b[1mIncompleteMatch:\x1b[0m\n" ++ showHint hint ++ "\x1b[1mContext:\x1b[0m\n" ++ show ctx ++ show span
    -- UnknownExprination term  -> "\x1b[1mUnknownExprination:\x1b[0m " ++ show term
