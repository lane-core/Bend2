...
import Core.Sort hiding (CApp, CLet)
...
-- Compilable Expr (only runtime-relevant constructs)
data CT
  = CVar String Int
  | CRef String
  | CLam String (CT -> CT)
...
-- Convert CT back to Expr for substitution
ctToExpr :: CT -> Expr
ctToExpr (CVar k i) = Var k i
ctToExpr _ = error "ctToExpr: only variables can be converted back"

-- Convert Expr to CT (erase types, keep runtime values)
termToCT :: Book -> Expr -> Int -> CT
termToCT book term dep = case term of
  Var k i -> CVar k i
  Ref k i -> CRef k
  Sub t -> termToCT book t dep
  Fix k f -> CFix k (\x -> termToCT book (f (ctToExpr x)) (dep + 1))
  Let k t v f -> CLet (termToCT book v dep) (\x -> termToCT book (f (ctToExpr x)) (dep + 1))
  Use k v f -> termToCT book (f v) dep -- Inline directly, no binding
  Set -> CEra
  Chk x _ -> termToCT book x dep
  Tst x -> termToCT book x dep
  Emp -> CEra
  EmpM -> CLam "x$" (const CEra)
  Uni -> CEra
  Bit -> CEra
  Nat -> CEra
  Lst _ -> CEra
  ...
  Val v -> CVal v
  Lam k _ f -> CLam k (\x -> termToCT book (f (ctToExpr x)) (dep + 1))
...
compileLambda var lam dep = do
  let (names, bodyExpr, newDep) = collectLambdas lam dep
  bodyName <- fresh
  bodyStmt <- ctToJS' False bodyName bodyExpr newDep
...
compileApp book fnName fnArgs isTail var app dep = do
  ...
  -- Tail recursive call
  if isTail && isRecCall fnName (length fnArgs) appFun appArgs
    then
